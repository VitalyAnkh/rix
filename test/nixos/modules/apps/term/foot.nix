...
with lib;
let
  foot = (evalConfig [{ modules.apps.term.foot.enable = true; }]);
in {
  ## The enable flag gates everything, which is the convention all 85 modules
  ## follow.

  testDisabledByDefault = {
    expr = (evalConfig []).modules.apps.term.foot.enable;
    expected = false;
  };
  ...
  testTerminalFollowsTermDefault = {
    expr = (evalConfig [{ modules.apps.term.default = "foot"; }])
  ...
  testXtermDesktopManagerFollowsTermDefault = {
    expr = map
      (t: (evalConfig [{ modules.apps.term.default = t; }])
