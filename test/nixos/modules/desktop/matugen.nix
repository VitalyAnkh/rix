# test/nixos/modules/desktop/matugen.nix --- tests for modules/desktop/matugen.nix
#
...
let
  ...
  bare = matugen [{
    modules.desktop.matugen.templates.example.input_path = "/in";
  }];
in {
  ## The config table.
  ...
  testTemplatesTableSurvivesWithNoTemplates = {
    expr = hasInfix "\n[templates]\n" (matugen [{
      modules.desktop.matugen.colors.brand = "#ff0000";
    }]);
    expected = true;
  };

  # The module has no enable option (see modules/desktop/matugen.nix), so this
  ...
  testBareHexColorIsNormalised = {
    expr = hasInfix ''
      ...
      color = "#ff0000"
      blend = true
    '' (matugen [{ modules.desktop.matugen.colors.brand = "#ff0000"; }]);
    expected = true;
  };
  ...
  testColorSubmoduleKeepsBlend = {
    expr = hasInfix ''
      ...
      blend = false
    '' (matugen [{
      modules.desktop.matugen.colors.accent = { color = "#00ff00"; blend = false; };
    }]);
    expected = true;
  };
  ...
  testTemplateRendersInputAndOutput = {
    expr = hasInfix ''
      ...
      output_path = "/out"
    '' (matugen [{
      modules.desktop.matugen.templates.example = {
        input_path = "/in";
        output_path = "/out";
      };
    }]);
  ...
  testLibrewolfCoversBothProfiles = {
    expr =
      let text = matugen [{ modules.desktop.browsers.librewolf.enable = true; }];
  ...
  testLibrewolfFollowsProfileName = {
    expr = hasInfix "/librewolf/librewolf/bob.default/chrome/userChrome.colors.css" (matugen [{
      ...
      modules.desktop.browsers.librewolf.profileName = "bob";
    }]);
    expected = true;
  };
}
